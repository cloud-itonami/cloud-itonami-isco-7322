(ns pressman.advisor
  "Pressman Advisor — proposing a print-shop scheduling/logistics
  coordination operation (log a work record, schedule a crew
  operation, flag a safety concern, coordinate an ink/paper-materials
  supply order) from a crew roster, shop registration and
  safety-reporting policy. Swappable mock/llm; the advisor ONLY
  proposes — `pressman.governor` independently gates every proposal
  and always escalates safety concerns and above-threshold supply
  orders. The advisor never proposes to directly finalize a
  press-run-execution decision (e.g. deciding to proceed with a
  specific printing press run) or to override a shop safety officer's
  judgment — those stay permanently out of this actor's scope.
  Modeled on cloud-itonami-isco-7223's advisor for the closely-related
  rotating-machinery safety-domain shape.
...
(defn- rationale-for [op worker-id shop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for press-time changeover at shop " shop-id)
...
(def ^:private system-prompt
  "You are a print-shop scheduling/logistics coordination advisor.
   ...
   propose to directly finalize a press-run-execution decision (e.g.
   deciding to proceed with a specific printing press run), or to
   override a shop safety officer's judgment — those are always out
   of this actor's scope; it coordinates print-shop scheduling/
   logistics only and never operates a printing press or authorizes
   press-run work itself. Safety concerns always require human
