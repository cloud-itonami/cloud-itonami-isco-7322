(ns pressman.governor
  "PressmanGovernor — the independent safety/scope layer gating every
  print-shop scheduling/logistics proposal an advisor may make for a
  print-shop crew. The governor never dispatches hardware itself,
  never operates a printing press on the shop floor, and never
  finalizes a press-run-execution decision (e.g. deciding to proceed
  with a specific printing press run) or overrides a shop safety
  ...
  SCHEDULING/LOGISTICS ONLY — it never operates the printing press
  itself). Modeled on cloud-itonami-isco-7223's machinist.governor for
  the closely-related rotating-machinery safety-domain shape.

  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    1. worker provenance     — the crew member must be independently
                                verified/registered before any action.
    2. shop provenance        — the print shop must be independently
                                verified/registered before any action.
    3. no-actuation           — proposal :effect must be :propose (the
                                governor never dispatches hardware and
                                never operates the printing press
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                press-run-execution decision (e.g.
                                deciding to proceed with a specific
                                printing press run), or to override a
                                ...
                                press run\" / \"authorize the press run
                                operation\" / \"override the shop
                                safety officer's judgment\", never as
                                bare nouns like \"press\" or \"ink\" or
                                ...
                                crew operation for press-time
                                changeover …\" or \"…routed for shop
...
(def ^:private scope-excluded-ops
  #{:finalize-press-run-decision :authorize-press-run-operation
    :proceed-with-press-run :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("press", "ink", "paper", "safety", "shop safety officer") — so
...
;; "press-time changeover" / "shop safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the press run" "proceed with the printing press run"
   "finalize the press run decision" "authorize the press run operation"
   "authorize the printing press run"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :no-actuation
             :detail "effect は :propose のみ許可（governor は印刷所作業を直接実行しない）"})
      ...
      (conj {:rule :scope-excluded-action
             :detail "印刷機の運転実行判断（press run の実行）の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `pressman.store/Store`. Pure — never mutates
  the store, never dispatches a print-shop operation."
