(ns pressman.actor
  "PressmanActor — the ISCO-08 7322 printers scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one
  print-shop-coordination operation request (intake -> advise ->
  govern -> decide -> commit/hold, with a human-approval interrupt
  for escalated proposals). No infinite internal loop; checkpointed
  per superstep so an interrupted run can resume after human
  sign-off. Modeled on cloud-itonami-isco-7223's machinist.actor for
  ...
  The unconditional invariant: the Pressman Advisor can never
  directly commit an operating record the PressmanGovernor refuses,
  and can never make this actor finalize a press-run-execution
  decision (e.g. deciding to proceed with a specific printing press
  run) or override a shop safety officer's judgment — every
  ...
  This actor coordinates SHOP SCHEDULING/LOGISTICS ONLY — it never
  operates the printing press itself or authorizes press-run work
  itself."
...
(defn build-graph
  "Build a compiled PressmanActor graph. `store` implements
  `pressman.store/Store`. `advisor` implements
  `pressman.advisor/Advisor` (defaults to `mock-advisor`).
